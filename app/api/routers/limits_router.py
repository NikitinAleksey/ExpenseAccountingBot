...
async def limits_waiting_for_sum(message: Message, state: FSMContext, texts: dict):
    ...
    return await message.reply(
        text=formatted_msg,
        reply_markup=InlineKeyBoard().create_kb(buttons=texts['inline_buttons']['limits_yes_no'])
    )


@limits_router.callback_query(StateFilter(LimitsStates.waiting_for_repeat_limits))
async def limits_waiting_for_repeat(callback: CallbackQuery, state: FSMContext, texts: dict):
