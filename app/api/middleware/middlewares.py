...
class DeletePreviousMSGMiddleware(BaseMiddleware):
    ...
    async def __call__(self, handler: Callable[[TelegramObject, Dict[str, Any]], Awaitable[Any]], event: TelegramObject,
        ...
        if self.storage.get(chat_id):
            ...
            self.log.info(f'Чат: {chat_id}. Чат пуст, создали очередь для сообщений, которые будут удалены.')
        if isinstance(event.message, Message):
            await self.storage[chat_id].put(event.message)
            self.log.info(f'Чат: {chat_id}. Входящее событие - сообщение. Добавили в очередь.')
        result = await handler(event, data)
