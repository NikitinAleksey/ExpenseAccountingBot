...
class UserController(BaseController):
    ...
    async def register_user(cls, tg_id: int, name: str, timezone: int):
        cls.log.info(f'Метод register_user. Попытка регистрации пользователя tg_id={tg_id}, name={name}, timezone={timezone}.')
        ...
        async with async_session as session:
            ...
            cls.log.debug(f'Метод register_user. Проверка наличия пользователя с tg_id={tg_id}.')
            if user:
                cls.log.info(f'Метод register_user. Пользователь с {tg_id=} уже зарегистрирован.')
                return
            new_user = cls._user(
                tg_id=tg_id,
                name=name,
            ...
            cls.log.info(f'Метод register_user. Пользователь с {tg_id=} успешно зарегистрирован.')
    ...
    async def update_user(cls, tg_id: int, name: str, timezone: int):
        ...
        async with async_session as session:
            ...
            cls.log.debug(f'Метод update_user. Новый данные пользователя: {updated_user}.')

            await cls._repository.update(
                session=session,
                item=updated_user
            )
            cls.log.info(f'Метод update_user. Данные пользователя {tg_id=} успешно обновлены.')
            return updated_user
