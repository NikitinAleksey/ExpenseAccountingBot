...
class FastReport:
    ...
    async def get_fast_report(
        ...
        cls.log.info(f'Метод get_fast_report. Запуск быстрого отчета для {tg_id=}, {start=}.')
        ...
        cls.log.debug(f'Метод get_fast_report. Получены данные: расходы={expenses}, лимиты={limits}.')
    ...
    async def _build_data_for_fast_report(
        ...
        cls.log.info(f'Метод build_data_for_fast_report. Сбор данных для tg_id={tg_id}, start={start}.')

        expenses = {}
        limits = {}
        async with async_session as session:
            for model in models:
                cls.log.debug(f'Метод build_data_for_fast_report. Получаем сумму для модели {model.__tablename__}.')
                ...
                cls.log.debug(f'Метод build_data_for_fast_report. Сумма для модели {model.__tablename__}: {expenses[model.__tablename__]}.')
            ...
            if limits_record:
                ...
                cls.log.debug(f'Метод build_data_for_fast_report. Лимиты для {tg_id=}: {limits}.')
            else:
                cls.log.warning(f'Метод build_data_for_fast_report. Лимиты для {tg_id=} не найдены.')
    ...
    def _build_fast_report_result(cls, expenses: dict, limits: dict, mapping: dict):
        ...
        cls.log.debug(f'Метод build_fast_report_result. Сформированы данные отчета, возвращаем пользователю.')
