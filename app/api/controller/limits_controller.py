...
class LimitsController(BaseController):
    ...
    async def init_limits(cls, tg_id: int):
        ...
        async with async_session as session:
            ...
            cls.log.info(f'Метод init_limits. Лимиты успешно инициализированы для tg_id={tg_id}.')
            return record

    @classmethod
    async def update_limit(cls, tg_id: int, article_name: str, article_value: str):
        cls.log.info(f'Метод update_limit. Обновление лимита для tg_id={tg_id}, article_name={article_name}, article_value={article_value}.')
        try:
            validated_data = LimitsValidator(amount=article_value, article=article_name)
            cls.log.info(f'Метод update_limit. Валидация успешна для: {validated_data.amount}.')
        ...
        async with async_session as session:
            ...
            cls.log.info(f'Метод update_limit. Текущий лимит для tg_id={tg_id}: {current_record}.')
            setattr(current_record, validated_data.article, validated_data.amount)
            updated_record = await cls._repository.update(
                session=session,
                item=current_record
            )
            cls.log.info(f'Метод update_limit. Лимит обновлен для tg_id={tg_id}: {updated_record}.')
