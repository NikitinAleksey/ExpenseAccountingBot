...
class ExpensesController(BaseController):
    ...
    def make_a_dict(cls, records: list[BaseArticle]):
        cls.log.info(f'Метод make_a_dict. Создаем словарь. ЭТУ ХЕРНЮ ПЕРЕПИСАТЬ!!!!!!!!!!!!!!!!!.')
    ...
    def extract_article_id_from_dict(cls, articles_dict: OrderedDict, article_value: str):
        cls.log.info(f'Метод extract_article_id_from_dict. Поиск id затраты для значения: {article_value}.')
        for key in articles_dict.keys():
            if articles_dict[key] == article_value:
                cls.log.info(f'Метод extract_article_id_from_dict. Найдено соответствие для {article_value}: {key}.')
                return key
        cls.log.info(f'Метод extract_article_id_from_dict. Не найдено соответствия для {article_value}.')

        return

    @classmethod
    async def add_expense(cls, tg_id: int, article_name: str, amount: str):
        cls.log.info(f'Метод add_expense. Запуск с параметрами: {tg_id=}, {article_name=}, {amount=}.')
        try:
            validated_data = InsertValidator(amount=amount, article=article_name)
            cls.log.info(f'Метод add_expense. Валидация успешна для: {validated_data.amount}.')
        ...
        async with async_session as session:
            new_expense_article = cls._model(
                user_id=tg_id,
                summ=validated_data.amount
            )
            record = await cls._repository.create(session=session, item=new_expense_article)
            cls.log.info(f'Метод add_expense. Успешно добавлена новая затратная статья: {tg_id=}, {validated_data.amount=}, {cls._model=}.')
            return record
    ...
    async def delete_expense(cls, articles_dict: OrderedDict, article_value: str):
        ...
        if not article:
            cls.log.warning(f'Метод delete_expense. Не найдено соответствующее значение для удаления: {article_value}.')
            return
    ...
    async def get_expenses(cls, tg_id: int, article_name: str):
        try:
            ...
            cls.log.info(f'Метод get_expenses. Валидация успешна для статьи: {validated_data.article}.')
